...
- [服务端 TypeScript 构建配置](file://server/tsconfig.build.json)
...
- [服务端 TypeScript 根配置:1-35](file://server/tsconfig.json#L1-L35)
- [服务端 TypeScript 构建配置:1-5](file://server/tsconfig.build.json#L1-L5)
...
SrvPkg --> TSBuild["tsconfig.build.json"]
...
## 结论
本项目通过配置包集中管理运行时参数，结合 pnpm workspace 实现跨包依赖与统一安装；前端与后端分别采用多配置文件策略保证类型检查与编译效率。建议在新增配置项时遵循“集中导出、按需扩展”的原则，并通过环境变量与 CI 管道实现多环境隔离与自动化校验。
